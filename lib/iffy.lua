...
--For castle.games
function iffy.newAtlasFromString(name, url, dataString, sw, sh)
	local t={}
	if url then
		if type(url)~='string' then
	...
	local i,sname,x,y,width,height=1
		...
		for line, newline in text:gmatch'([^\r\n]*)([\r\n]*)' do
			print(line)
		--READ XML FILE ('i' means the line number)
			if i>1 and line~="</TextureAtlas>" and line~="" then
				
				_, sname = string.match(line, "n=([\"'])(.-)%1")
				...
				_, x = string.match(line, "x=([\"'])(.-)%1")
				_, y = string.match(line, "y=([\"'])(.-)%1")
				_, width = string.match(line, "w=([\"'])(.-)%1")
				_, height = string.match(line, "h=([\"'])(.-)%1")
